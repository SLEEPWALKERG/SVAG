...
class MWZDataSet(pl.LightningDataModule):
    ...
    def setup(self, stage):
        if stage == "fit" or stage is None:
            with open("../data/val_data/{}/train.json".format(self.args.mwz_ver), encoding="utf-8") as f:
                self.train = json.load(f)
            if self.args.level_1 != "":
                with open("../data/val_data/{}/{}.json".format(self.args.mwz_ver, self.args.level_1),
                          encoding="utf-8") as f:
                    self.train.extend(json.load(f))
            if self.args.level_2 != "":
                with open("../data/val_data/{}/{}.json".format(self.args.mwz_ver, self.args.level_2),
                          encoding="utf-8") as f:
                    self.train.extend(json.load(f))
            with open("../data/val_data/{}/dev.json".format(self.args.mwz_ver), encoding="utf-8") as f:
                self.dev = json.load(f)
        if stage == 'test' or stage is None:
            with open("../data/val_data/{}/test.json".format(self.args.mwz_ver), encoding="utf-8") as f:
